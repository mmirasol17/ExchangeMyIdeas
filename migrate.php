 ...
 *
 * WHY THIS IS BROWSER-TRIGGERED, NOT FULLY AUTOMATED IN CI:
 *   InfinityFree does not allow remote MySQL connections, so a GitHub Actions
 *   runner cannot reach the database to run migrations directly. It also
 *   serves an anti-bot challenge to non-browser HTTP requests, which blocks
 *   curl-ing this endpoint from CI. So the flow is:
...
require_once('config.php'); // provides $conn and, via config.local.php, $migrateKey
